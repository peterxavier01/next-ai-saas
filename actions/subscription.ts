...
import { Plan, Subscription, WebhookEvent, Prisma } from "@prisma/client";
...
export async function syncPlans() {
  ...
  const productVariants: Plan[] = await prismadb.plan.findMany();
  ...
  const checkout = await createCheckout(
    ...
    variantId,
    {
      ...
      productOptions: {
        ...
        receiptThankYouNote: "Thank you for signing up to Genius!",
      },
    },
  );
...
export async function hasWebhook() {
  ...
  // Check if WEBHOOK_URL ends with a slash. If not, add one.
...
export async function processWebhookEvent(webhookEvent: WebhookEvent) {
  ...
  const dbWebhookEvent = await prismadb.webhookEvent.findMany({
  ...
  if (!webhookHasMeta(eventBody)) {
    ...
    if (webhookEvent.eventName.startsWith("subscription_payment_")) {
      // Save subscription invoices; eventBody is a SubscriptionInvoice. Not implemented YET.
    } else if (webhookEvent.eventName.startsWith("subscription_")) {
      // Save subscription events; obj is a Subscription
      const attributes = eventBody.data.attributes;
      const variantId = attributes.variant_id as string;

      const plan = await prismadb.plan.findMany({
      ...
      if (plan.length < 1) {
        ...
        const updateData: Omit<Subscription, "id"> = {
          ...
          userId: userId,
          planId: plan[0].id as number,
        };

        // Create/update subscription in the database
        try {
          // First, check if the UserApiLimit exists
          ...
          if (!userApiLimit) {
            ...
            // Now we can create the Subscription. This is to prevent an error if a user decides to subscribe first without testing any API generation initially.
          ...
          // If the UserApiLimit exists, update the Subscription
      ...
      // Save orders; eventBody is an "Order". Not implemented YET.
    } else if (webhookEvent.eventName.startsWith("license_")) {
      // Savve license keys; eventBody is a "License key". Not implemented YET.
    }

    // Update the webhook event in the database.
    await prismadb.webhookEvent.update({
      where: { id: webhookEvent.id },
      data: {
        processed: true,
        processingError,
...
export async function storeWebhookEvent(
  ...
  const returnedValue = await prismadb.webhookEvent.create({
    data: {
      id,
      eventName,
      processed: false,
      body: body === null ? Prisma.JsonNull : body,
    },
  });
...
export async function getUserSubscriptions() {
  ...
  const userSubscriptions: Subscription[] =
    await prismadb.subscription.findMany({
      where: { userId: userId },
    });
...
export async function changePlan(currentPlanId: number, newPlanId: number) {
  ...
  const newPlan = await prismadb.plan.findUnique({
    where: { id: newPlanId },
  });
